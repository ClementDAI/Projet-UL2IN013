...
class Controller:
    ...
    def update(self): 
        ...
        if self.action == None:
            self.strat = input("Entrez une action : 0 pour Avancer, 1 pour tourner, 2 pour carré, 3 pour s'approcher d'un mur, 4 hexagone, Dessine(True) et Dessine(False) qui set le crayon, change_couleur: ")
            if self.strat == "0":
                ...
                valeur = float(input("Valeur (positif) du coté du hexagone à tracer par le robot : "))
                self.action = Hexagone(valeur, self.trad)
                self.action.start()
